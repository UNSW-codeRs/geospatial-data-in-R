...
.plot-callout[
```{r jbm-plot-callout, ref.label="jbm-plot", fig.callout=TRUE}
```
]

---

```{r large-plot-full-output, ref.label="jbm-plot", echo=FALSE}
```

---

**Spatial autocorrelation** is a measure of how similar are observations that are close together. We use a **semivariogram** to represent this.
...
.pull-right[
```{r variogram-out, ref.label="variogram", echo=FALSE, fig.dim=c(4.8, 4.5), out.width="100%"}
```
]
