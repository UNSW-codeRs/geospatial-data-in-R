...
<div class="my-footer"><span>JR Ferrer-Paris </span></div>
...
.pull-left[
We can transform this prediction into a `raster` object
...
.pull-right[
And export this to a raster file in any format we want:
```{r, eval=TRUE,echo=TRUE}
writeRaster(r_ik,
...
```{r out.width='100%', fig.height=4, eval=require('leaflet'),warning=FALSE,message=FALSE}
library(leaflet)
leaflet() %>%
  addTiles() %>%
  addRasterImage(r_ik,colors='BuGn',opacity=0.75) %>%
  addCircles(data=data_ll,radius=~Tree.cover)
```
