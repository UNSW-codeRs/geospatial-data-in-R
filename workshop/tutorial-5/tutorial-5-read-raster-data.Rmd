...
I've preloaded the three key packages for this tutorial with:
...
This file is preloaded in this `learnr` tutorial, but if you want to replicate these steps in an R session, you can either download to a temporary folder:

```{r, eval=FALSE}
wdpa_query <- wdpa_fetch("BTN") 
```

Or you need to locate a local data folder for storing the data between sessions. For example, using package `here`:
...
We will clean the data set using the function `wdpa_clean()`. For more information on the data cleaning procedures applied here, see the help page of that function. 
...
Feel free to check the other tutorials in this workshop at the [UNSW codeRs' geospatial-data repository](https://github.com/UNSW-codeRs/geospatial-data-in-R).
